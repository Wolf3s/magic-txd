...
{
    typedef StaticPluginClassFactory <NativeImagePrivate> nativeImgFact_t;
    ...
    AINLINE nativeImageTypeManager* GetNativeImageTypeManagerByName( const char *natImgName ) const
    {
        ...
        LIST_FOREACH_BEGIN( nativeImageTypeManager, this->formatsList.root, manData.node )
            ...
            if ( stricmp( item->manData.imgType->name, natImgName ) == 0 )
            {
                return item;
            }

            // We also want to allow checking against the friendly name.
            if ( stricmp( item->manData.friendlyName, natImgName ) == 0 )
            {
                return item;
            }
...
extern nativeImageEnvRegister_t nativeImageEnvRegister;
