            ...
            // This is a versioned struct, so we have to be careful.
            {
                ...
                else
                {
                    ...
                    if ( !isCompressed )
                    {
                        ...
                        {
                            throw RwException( "failed to set format mapping for GC native texture (old version" );
                        }
                    }
...
void gamecubeNativeTextureTypeProvider::GetPixelDataFromTexture( Interface *engineInterface, void *objMem, pixelDataTraversal& pixelsOut )
{
    ...
    if ( !gotFormat )
    {
        ...
        throw RwException( "failed to get output raster format for GC native texture pixel fetch" );
    }
...
void gamecubeNativeTextureTypeProvider::SetPixelDataToTexture( Interface *engineInterface, void *objMem, const pixelDataTraversal& pixelsIn, texNativeTypeProvider::acquireFeedback_t& acquireFeedback )
{
    ...
    // Make sure that the texel data obeys size rules.
    {
        ...
        {
            throw RwException( "verification of size rules failed in GC native texture pixela acquisition" );
        }
    }
    ...
    else if ( srcCompressionType == RWCOMPRESS_NONE )
    {
        if ( srcPaletteType != PALETTE_NONE )
        {
            ...
            else
            {
                ...
                throw RwException( "invalid palette type in GC native texture texel acquisition" );
            }
    ...
    {
        throw RwException( "invalid compression type of texel data in GC native texture texel acquisition" );
    }

    if ( !gotFormat )
    {
        throw RwException( "failed to determine destination internalFormat for GC native texture texel acquisition" );
    }
    ...
    for ( size_t mip_index = 0; mip_index < mipmapCount; mip_index++ )
    {
        ...
        try
        {
            ...
            nativeTex->mipmaps.push_back( std::move( dstLayer ) );
        }
        catch( ... )
        {
