...

    {
        ...
        try
        {
            if ( gcNativeBlock.getBlockID() == CHUNK_STRUCT )
            {
                ...
                uint32 header_hasAlpha = 0;
                {
                        ...
                        // Read the raster format flags.
                        {
                            ...
                            if ( header_internalFormat != GVRFMT_CMP )
                            {
                                if ( header_palettePixelFormat != GVRPIX_NO_PALETTE )
                                {
                                    ...
                                    {
                                        engineInterface->PushObjWarningVerb( theTexture, "has a raster format descriptor that does not match its palettePixelFormat parameter" );
                                    }
                                }
                                else
                                {
                                    ...
                                    {
                                        engineInterface->PushObjWarningVerb( theTexture, "has a raster format descriptor that does not match its internalFormat parameter" );
                                    }
                                }
                            }
                    ...
                    else
                    {
                        ...
                        if ( paletteType != PALETTE_NONE )
                        {
                            ...
                            {
                                throw RwException( "invalid palette format in GC native texture deserialization" );
                            }

                            if ( header_isCompressed )
                            {
                                throw RwException( "encountered impossible GC native texture that has palette and compression enabled" );
                            }
                        ...
                        if ( !couldMapFormat )
                        {
                            throw RwException( "invalid GC native texture format mapping (old version)" );
                        }
                    }
                }
                ...
                // Verify properties.
                {
                    ...
                    // - depth
                    {
                        if ( depth != gcDepth )
                        {
                            ...
                            {
                                engineInterface->PushObjWarningVerb( theTexture, "has an invalid depth (ignoring)" );
                            }
                    ...
                    // - palette type.
                    {
                        ...
                        {
                            engineInterface->PushObjWarningVerb( theTexture, "has an invalid palette type (ignoring)" );
                ...
                uint32 mip_clusterWidth, mip_clusterHeight;
                ...
                if ( isGVRNativeFormatRawSample( internalFormat ) )
                {
                    ...
                    {
                        throw RwException( "fatal: failed to get cluster-props for GC native texture internalFormat" );
                    }
                }
