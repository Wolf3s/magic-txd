...
inline uint32 getDDSRasterDataRowSize( uint32 surfWidth, uint32 depth )
{
...
inline static const char* getDDSPixelFormatTypeName( eDDSPixelFormatType type )
{
...
struct ddsNativeImageFormatTypeManager : public nativeImageTypeManager
{
    ...
    {
        inline ddsNativeImage( Interface *engineInterface )
        {
        ...
        inline ddsNativeImage( const ddsNativeImage& right )
        {
    ...
    void WriteToNativeTexture( Interface *engineInterface, void *imageMem, const char *nativeTexName, void *nativeTexMem, acquireFeedback_t& feedbackOut ) const override
    {
        ...
        else if ( formatType == eDDSPixelFormatType::FMT_RGB )
        {
            ...
            if ( !dds_hasBitDepth )
            {
                throw RwException( "no valid bit depth to map DDS image to RGB format" );
            }
            ...
            {
                throw RwException( "failed to map RGB DDS file to D3DFORMAT" );
            }
        }
        else if ( formatType == eDDSPixelFormatType::FMT_YUV )
        {
            ...
            if ( !dds_hasBitDepth )
            {
                throw RwException( "no valid bit depth to map DDS image to YUV format" );
            }
            ...
            if ( !dds_hasValidFormat )
            {
                throw RwException( "failed to map YUV DDS file to D3DFORMAT" );
            }
        }
        else if ( formatType == eDDSPixelFormatType::FMT_LUM )
        {
            ...
            if ( !dds_hasBitDepth )
            {
                throw RwException( "no valid bit depth to map DDS image to LUM format" );
            }
            ...
            if ( !dds_hasValidFormat )
            {
                throw RwException( "failed to map LUM DDS file to D3DFORMAT" );
            }
        }
        else if ( formatType == eDDSPixelFormatType::FMT_ALPHA )
        {
            ...
            if ( !dds_hasBitDepth )
            {
                throw RwException( "no valid bit depth to map DDS image to alpha format" );
            }
            ...
            if ( !dds_hasValidFormat )
            {
                throw RwException( "failed to map alpha DDS file to D3DFORMAT" );
            }
        }
        else if ( isDDSPixelFormatPalette( formatType ) )
        {
            if ( !dds_hasBitDepth )
            {
                throw RwException( "no valid bit depth to map DDS image to palette format" );
            }
        ...
        {
            throw RwException( "could not map DDS file to a valid native raster representation" );
        }
        ...
        {
            throw RwException( "unsupported native texture type in DDS native image color data acquisition" );
        }
        ...
        d3dpublic::nativeTextureFormatHandler *dds_usedFormatHandler = NULL;
        ...
        if ( isDirect3D9 )
        {
            ...
            size_t mipmapCount = ddsImage->mipmaps.size();
            ...
            eByteAddressingMode srcByteAddrMode, dstByteAddrMode;
            ...
            bool dds_hasAlpha = false;
            {
                ...
                if ( isDXTCompressed || dds_d3dRasterFormatLink || dds_usedFormatHandler != NULL )
                {
                    ...
                    else
                    {
                        ...
                        else if ( dds_usedFormatHandler != NULL )
                        {
                        ...
                        if ( canRasterFormatHaveAlpha( mipRasterFormat ) )
                        {
                            for ( uint32 n = 0; n < mipmapCount; n++ )
                            {
                                ...
                                if ( !directCheck )
                                {
                                    ...
                                    try
                                    {
                                        dds_usedFormatHandler->ConvertToRW( srcTexels, mipWidth, mipHeight, dstStride, srcDataSize, dstTexels );
                                    }
                                    catch( ... )
                                    {
                                        engineInterface->PixelFree( dstTexels );
        ...
        else
        {
            ...
            ddsNativeImage::mipmaps_t tmpMipmapArray;
            ...
            try
            {
                if ( dds_usedFormatHandler )
                {
                    ...
                    for ( size_t n = 0; n < mipmapCount; n++ )
                    {
                        ...
                        uint32 dstRowSize = getRasterDataRowSize( layerWidth, frmDepth, frmRowAlignment );
                        ...
                        if ( !dstTexels )
                        {
                            throw RwException( "failed to allocate destination buffer for packed D3DFORMAT unpacking" );
                        }
                ...
                {
                    throw RwException( std::string( "failed to push DDS native image color data to " ) + nativeTexName + " native texture due to unsupported DDS format" );
                }
    ...
    void ReadFromNativeTexture( Interface *engineInterface, void *imageMem, const char *nativeTexName, void *nativeTexMem, acquireFeedback_t& feedbackOut ) const override
    {
        ...
        ddsNativeImage::mipmaps_t srcLayers;
        ...
        d3dpublic::nativeTextureFormatHandler *srcAnonymousFormatHandler = NULL;
        ...
        else if ( strcmp( nativeTexName, "Direct3D8" ) == 0 )
        {
            NativeTextureD3D8 *nativeTex = (NativeTextureD3D8*)nativeTexMem;
        ...
        {
            throw RwException( "unsupported native texture type in DDS image texel acquisition" );
        }
        ...
        try
        {
            ...
            if ( !src_hasD3DFORMAT || srcHasRasterLink )
            {
                ...
                if ( dstCompressionType == RWCOMPRESS_NONE )
                {
                    if ( dstPaletteType != PALETTE_NONE )
                    {
                        ...
                        {
                            throw RwException( "unsupported palette type in DDS native image texel acquisition from native texture" );
                        }
                    ...
                    else
                    {
                        if ( !src_hasD3DFORMAT )
                        {
                            ...
                            ePaletteType _unused_palette_type = PALETTE_NONE;

                            d3d9::convertCompatibleRasterFormat(
                                true,
                                dstRasterFormat, dstColorOrder, dstDepth, _unused_palette_type,
                                dds_d3dFormat
                            );
                        }
                    }
                ...
                else if ( !src_hasD3DFORMAT )
                {
                    ...
                    {
                        throw RwException( "unknown compression type in DDS native image texel acquisition from native texture" );
                    }
                }
                ...
                try
                {
                    ...
                    {
                        ddsNativeImage::mipmaps_t convLayers;
                        ...
                        try
                        {
                            for ( size_t n = 0; n < mipmapCount; n++ )
                            {
                                ...
                                convLayers.push_back( std::move( newLayer ) );
                            }
                        }
                        catch( ... )
                        {
            ...
            else
            {
                ...
                if ( gotDepth && hasFormatPitch )
                {
                    ...
                    const eByteAddressingMode d3dByteAddr = eByteAddressingMode::MOST_SIGNIFICANT;
    ...
    )
    {
        ...
        else if ( isPitch )
        {
            ...
            mainLayerDataSize = getRasterDataSizeByRowSize( mainLayerPitchOrLinearSize, surfHeight );
        }
        else
        {
    ...
    void ReadNativeImage( Interface *engineInterface, void *imageMem, Stream *inputStream ) const override
    {
        ...
        {
            throw RwException( "DDS files with depth are not supported (DDSD_DEPTH)" );
        }
        ...
        eDDSPixelFormatType formatType;
        {
            ...
            {
                throw RwException( "failed to determine DDS native image color format (RGB,YUV,LUM,alpha,fourCC,pal4,pal8)" );
            }
                 ...
                 hasPal8 && formatType != eDDSPixelFormatType::FMT_PAL8 )
            {
                ...
                std::string warningMsg = "ambiguous DDS native image format: ";

                if ( hasRGB )
                {
                    warningMsg +=
                        (std::string)getDDSPixelFormatTypeName( eDDSPixelFormatType::FMT_RGB ) + " ";
                }

                if ( hasYUV )
                {
                    warningMsg +=
                        (std::string)getDDSPixelFormatTypeName( eDDSPixelFormatType::FMT_YUV ) + " ";
                }
            
                if ( hasLum )
                {
                    warningMsg +=
                        (std::string)getDDSPixelFormatTypeName( eDDSPixelFormatType::FMT_LUM ) + " ";
                }

                if ( hasAlphaOnly )
                {
                    warningMsg +=
                        (std::string)getDDSPixelFormatTypeName( eDDSPixelFormatType::FMT_ALPHA ) + " ";
                }

                if ( hasPal4 )
                {
                    warningMsg +=
                        (std::string)getDDSPixelFormatTypeName( eDDSPixelFormatType::FMT_PAL4 ) + " ";
                }

                if ( hasPal8 )
                {
                    warningMsg +=
                        (std::string)getDDSPixelFormatTypeName( eDDSPixelFormatType::FMT_PAL8 ) + " ";
                }

                warningMsg += std::string( "formats specified; " ) + getDDSPixelFormatTypeName( formatType ) + " format selected";

                engineInterface->PushWarning( std::move( warningMsg ) );
            }
        }
        ...
        uint32 maybeMipmapCount = ( hasMipmapCount ? header.dwMipmapCount : 1u );
        ...
        // The palette is always in 32bit RGBA color.
        {
                 ...
                 formatType == eDDSPixelFormatType::FMT_PAL8 )
            {
                ...
                if ( !paletteData )
                {
                    throw RwException( "failed to allocate palette buffer in DDS native image deserialization" );
                }
                ...
                try
                {
                    ...
                    {
                        throw RwException( "failed to read palette buffer in DDS native image deserialization" );
                    }
                }
                catch( ... )
                {
        ...
        while ( mip_index < maybeMipmapCount )
        {
            ...
            if ( !mipTexels )
            {
                throw RwException( "failed to allocate DDS native image mipmap buffer" );
            }
            ...
            try
            {
                ...
                {
                    throw RwException( "failed to read DDS native image mipmap buffer" );
                }
                ...
                ddsImage->mipmaps.push_back( std::move( newLayer ) );
            }
            catch( ... )
            {
                engineInterface->PixelFree( mipTexels );
    ...
    void WriteNativeImage( Interface *engineInterface, const void *imageMem, Stream *outputStream ) const override
    {
        ...
        if ( mipmapCount == 0 )
        {
            throw RwException( "attempt to write empty DDS native image" );
        }
        ...
        {
            throw RwException( "unknown DDS native image color format in DDS serialization" );
        }
        ...
        uint32 baseLayerPitchOrLinearSize;
        {
            ...
            else
            {
                if ( bitDepth == 0 )
                {
                    throw RwException( "invalid bit depth in DDS native image serialization" );
                }

                baseLayerPitchOrLinearSize = getDDSRasterDataRowSize( baseLayer.width, bitDepth );
            }
        }
    ...
    inline void Initialize( EngineInterface *engineInterface )
    {
        ...
        RegisterNativeImageType(
            engineInterface,
            this,
            "DDS", sizeof( ddsNativeImage ), "DirectDraw Surface",
            dds_file_extensions, _countof( dds_file_extensions ),
            dds_supported_nat_textures, _countof( dds_supported_nat_textures )
        );
    }
...
static PluginDependantStructRegister <ddsNativeImageFormatTypeManager, RwInterfaceFactory_t> ddsNativeImageFormatTypeManagerRegister;

void registerDDSNativeImageFormatEnv( void )
{
    ddsNativeImageFormatTypeManagerRegister.RegisterPlugin( engineFactory );
}
